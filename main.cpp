...
void SetUp(Player &player1, Player &player2, Player &player3,
	...
	// player drawcard 2 tile 3 time (6tile)
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 2; j++)
		...
		for (int j = 0; j < 2; j++)
			DrawCard(wallhand, temphand[2]);
	}
	// player draw the 7th tile
	DrawCard(wallhand, temphand[0]);
	DrawCard(wallhand, temphand[1]);
	DrawCard(wallhand, temphand[2]);
	...
	Sort(sortby, temphand[2]);
	// player1 be bookmaker
	if (player1.bookmaker){
		player1.hand = temphand[0];
		player2.hand = temphand[1];
		player3.hand = temphand[2];
	// palyer2 be bookmaker
	}else if (player2.bookmaker){
		player2.hand = temphand[0];
		player3.hand = temphand[1];
		player1.hand = temphand[2];
	// player3 be bookmaker
	}else if (player3.bookmaker){
		player3.hand = temphand[0];
		player1.hand = temphand[1];
		player2.hand = temphand[2];
	}
}
...
class PrintAll {
	public:
		...
		void PrintAll_SetNameAndMap(string player1, string player2, string player3,
																string wall, string river, 
		...
		void PrintHand(int whitchplayer, vector<string> hand, bool shownumber) {
			// -1 = river, 0 = wall, 1 = player1, 2 = player2, 3 = player3
			switch (whitchplayer) { 
				case -1:
					cout << this->river;
					break;
		...
		void PrintAll_Print(Player player1, Player player2, Player player3,
			...
			// print player3 and player3meld
			cout << "============================================================" << endl;
			PrintMeldHand(3, player3.meldhand);
			cout << "------------------------------------------------------------" << endl;
			PrintHand(3, player3.hand, false);
			cout << "============================================================" << endl;
		};
	private:
		// declare type
...
class Kong {
	public:
		...
		void CheckConcealedAndSmall(Player &player, vector<string> &wall, 
				bool &top, bool &winonthewalltail){
			char key;
			// Concealed
...
class Score {
	...
	public:
		...
		void SetAccordingTo(vector<string> sortby, Player player, 
				...
				bool winonthewalltail, int continuetobookmaker) {
			this->sortby = sortby;
...
void ContinueToBookmakerOrChange(int &continuetobookmaker, int winner,
														     bool &player1bookmaker, bool &player2bookmaker,
																 bool &player3bookmaker, int tempbookmaker, 
																 int &gameturn){
	...
	if (winner == tempbookmaker){
		...
		switch (tempbookmaker){
			...
			case 2:
				...
				break;
			case 3:
				player3bookmaker = !player3bookmaker;
				gameturn = 2;
				break;
		}
...
void CheckKongTwoTime(vector<string> sortby, map<string,string> tile, 
		...
		bool &gameloop, bool &top, bool &winonthewalltail, 
		bool &tsumo, string &eye){
	// declare type
	CheckWin cw;
	CheckMeld cm;
	...
	for(int i = 0; i < 2;i++){
		cm.CheckConcealedAndSmall(player, wallhand, top, winonthewalltail);
...
void PlayerDiscardAndOtherCheckWinOrMeld(
		map<string,string> tile, vector<string> sortby, Player &player1,
		Player &player2, Player &player3, vector<string> &wallhand, 
		vector<string> &riverhand, int &gameround, int gameturn, bool &gameloop,
	 	bool &gameflow,	bool &top, bool &winonthewalltail, bool &tsumo, string &eye){
	// declare class	
	bool ismeld = false;
	PrintAll pa;
	CheckWin cw;
	...
	pa.PrintAll_SetNameAndMap(player1.name, player2.name, player3.name, "牌山", "牌河", tile);
	// cw class set tile
	cw.CheckWin_SetTile(sortby);
	// cm class set tile
	cm.SetTile(tile);
	...
	// print all hand
	pa.PrintAll_Print(player1, player2, player3, wallhand, riverhand);
	...
	if (!gameloop){
		// set eye
		cw.GetEye(eye);
		// set gameturn to 1 mean turn to player2
		gameturn = player2.code;
	// player2 isn't ron
	}else{ 
		// check player3 ron
		gameloop = cw.CheckWin_Check(player3, riverhand);
		// player3 is ron
		if (!gameloop){
			// set eye
			cw.GetEye(eye);
			// set gameturn to 2 mean turn to player3
			gameturn = player3.code;
		// player3 isn't ron
		}else{
			// check player3 has meld (pong or kong) with riverhand last
			ismeld = cm.Check(riverhand, player3, false);
			// player3 has meld and player3 isn't tenpai 
			if (ismeld && !player3.tenpai){
				// ask player3 want meld or not
				cm.MeldAskAndSelect();
				// make meld (pong or kong) from player3hand 
				// and riverhand last one to player3meldhand
				cm.BeenMeld(player3, riverhand, wallhand, top);
				// player3concealed is concealed and player3meldhand is empty
				if (player3.concealed && !player3.meldhand.empty())
					player3.concealed = !player3.concealed;
				// key == p mean pong 
				if (cm.key == 'p'){
					// set gameturn to player3
					gameturn = player3.code;
				// key == k mean kong and check is win on the wall tail
					...
					if (!gameloop){
						...
						// set tsumo to true
						tsumo = true;
						// set gameturn == 2(player3)
						gameturn = player3.code;
					// player3 isn't winning
					}else{
						// try to chek the kong again 
						cm.CheckConcealedAndSmall(player3, wallhand, top, winonthewalltail);
						// check win after kong again
						gameloop = cw.CheckWin_Check(player3, riverhand);
						// player3 is winning after kong again
						if (!gameloop){
							// set eye
							cw.GetEye(eye);
							// set tsumo to true
							...
							// set gameturn == 2(player3)
							gameturn = player3.code;
						// player3 isn't winning after kong again
						}else{
							// set winonthewalltail to false mean isn't winning
							winonthewalltail = !winonthewalltail;
							goto Player2Check;
				...
				// key == s mean skip
						...
						goto Player2Check;
			// player3 doesn't have meld or player3 is tenpai
			}else{
Player2Check:
				// check player2 has meld (chi, pong or kong) with riverhand last
				ismeld = cm.Check(riverhand, player2, true);
				// player2 has meld and player2 isn't tenpai
				if (ismeld && !player2.tenpai){
					// ask player2 want meld or not
					cm.MeldAskAndSelect();
					// make meld (chi, pong or kong) from player2hand 
					// and riverhand to player2meldhand
					cm.BeenMeld(player2, riverhand, wallhand, top);
					// player2concealed is concealed and player2meldhand is empty 
					if (player2.concealed && !player2.meldhand.empty())
						player2.concealed = !player2.concealed;
					// key == s mean skip and discard
					if (cm.key == 's') {
						goto Player2Discard;
					// key == k mean kong and check is win on the wall tail
					}else if (cm.key == 'k'){
						...
						// player2 isn't winning
							...
							// try to chek the kong again 
							cm.CheckConcealedAndSmall(player2, wallhand, top, winonthewalltail);
							// check win after check kong again
							gameloop = cw.CheckWin_Check(player2, riverhand);
							// player2 is winning after kong again
							if (!gameloop){
								// set eye 
								cw.GetEye(eye);
								// set tsumo to true
								tsumo = true;
							// palyer2 isn't winning after kong again
							}else
								// set winonthewalltail to false mean isn't winning
								winonthewalltail = !winonthewalltail;
						}
					}
				// player2 doesn't have meld or player2 isn't tenpai
				}else{
					// draw card
					DrawCard(wallhand, player2.hand);
Player2Discard:
					...
					// wall isn't empty no flow
						...
						// try to check concealed or small kong
						CheckKongTwoTime(sortby, tile, player2, wallhand, riverhand, 
							gameloop, top, winonthewalltail, tsumo, eye);
...
int main() {
	...
	bool top = true;
	// int
	int gameturn = 0; // #0 = player1, #1 = player2, #2 = player3
	int gameround = 1;
	int winner = 0;
	int continuetobookmaker = 0;
	int tempbookmaker = 0;
	...
	s.SetTile(tile);
ReStart:
	SetUp(player1, player2, player3, tile, wallhand, sortby);
	// print all hand
	pa.PrintAll_Print(player1, player2, player3, wallhand, riverhand);
		...
		switch(gameturn){
			case 0:
				CheckKongTwoTime(sortby, tile, player1, wallhand,
						riverhand, gameloop, top, winonthewalltail, tsumo, eye);
				break;
			case 1:
				...
				break;
			case 2:
				CheckKongTwoTime(sortby, tile, player3, wallhand,
						riverhand, gameloop, top, winonthewalltail, tsumo, eye);
				break;
		}
	}
Head:
	while (gameloop) {
		...
		gameturn != 2 ? gameturn += 1 : gameturn -= 2;
		// player3 discard and player1 check tsumo or ron
		if (gameturn == 0) {
			PlayerDiscardAndOtherCheckWinOrMeld(tile,sortby, player3, player1, 
					player2, wallhand, riverhand, gameround, gameturn, gameloop,
					gameflow, top, winonthewalltail, tsumo, eye);
		// player1 discard and player2 check tsumo or ron
		} else if (gameturn == 1) {
			PlayerDiscardAndOtherCheckWinOrMeld(tile,sortby, player1, player2, 
					player3, wallhand, riverhand, gameround, gameturn, gameloop,
					gameflow, top, winonthewalltail, tsumo, eye);
		// player2 discard and player3 check tsumo or ron
		} else if (gameturn == 2) {
			PlayerDiscardAndOtherCheckWinOrMeld(tile, sortby, player2, player3, 
					player1, wallhand, riverhand, gameround, gameturn, gameloop, 
					gameflow, top, winonthewalltail, tsumo, eye);
		}
	}
	switch (gameturn) {
		case 0:
			s.SetAccordingTo(sortby, player1, riverhand, eye, gameround, gameturn,
					tsumo, lastwalltile, winonthewalltail, 
					continuetobookmaker);
			winner = 1;
			break;
		case 1:
			...
			break;
		case 2:
			s.SetAccordingTo(sortby, player3, riverhand, eye, gameround, gameturn,
					tsumo, lastwalltile, winonthewalltail,
					continuetobookmaker);
			winner = 3;
			break;
	...
	if (player3.bookmaker && winner != 3) {
		...
		ContinueToBookmakerOrChange(continuetobookmaker, winner, player1.bookmaker, 
				player2.bookmaker, player3.bookmaker, tempbookmaker, gameturn);
		tempbookmaker = 0;
		winner = 0;
		// class 
		pa = PrintAll();
